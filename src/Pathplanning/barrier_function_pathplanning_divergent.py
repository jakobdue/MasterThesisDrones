...
#original_sequences = generate_circle_crossing(num_drones=19)
...
def total_path_energy(paths:list[Path]): 
    ...
    for path in paths:
        for k in range(len(path)-1): # Compare with initial path x0
            diff = sub_vecs(path[k], unflatten_paths(x0, len(paths))[paths.index(path)][k])
    ...
    return (total_path_length_energy, total_barrier_energy, total_diversion_energy)
...
def gradient_barrier_energy(paths:list[Path]):
        ...
        for k in range(T):
            if k == 0 or k == T-1:
                ...
                grad_divert[i][k] = scale_vec(
                    ...
                    sub_vecs(
                        paths[i][k],
                        unflatten_paths(x0, len(paths))[i][k]
                    )
                )
    ...
    return (grad_smooth, grad_barrier, grad_divert)


    
def objective(x: np.ndarray, num_paths: int):
    paths = unflatten_paths(x, num_paths)
    path_energy, barrier_energy, diversion_energy = total_path_energy(paths)
    return path_energy + barrier_energy + diversion_energy

def objective_grad(x: np.ndarray, num_paths: int):
    paths = unflatten_paths(x, num_paths)
    grad_smooth, grad_barrier, grad_divert = gradient_barrier_energy(paths)

    grad = []
    for i in range(len(paths)):
        for k in range(T):
            g = add_vecs(add_vecs(grad_smooth[i][k], grad_barrier[i][k]), grad_divert[i][k])
            grad.extend(g)
...
final_path_energy, final_barrier_energy, final_diversion_energy = total_path_energy(new_paths)
print(f"Final path energy: {final_path_energy:.4f}")
print(f"Final barrier energy: {final_barrier_energy:.4f}")
print(f"Final diversion energy: {final_diversion_energy:.4f}")
