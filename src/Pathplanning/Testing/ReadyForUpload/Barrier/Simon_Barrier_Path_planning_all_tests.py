...
MAX_STEP = eta_safety_distance / 2
T = 10
...
def step_constraints(x, num_paths):
    ...
    for path in paths:
        for k in range(T - 1):
            ...
            vals.append(
                squared_vec_length(diff)
            )

    return np.array(vals)

def step_constraint_jac(x, num_paths):
    paths = unflatten_paths(x, num_paths)

    n_constraints = num_paths * (T - 1)
    n_vars = len(x)
    ...
    for i in range(num_paths):
        for k in range(T - 1):
            ...
            dx = x[p2_idx]   - x[p1_idx]
            dy = x[p2_idx+1] - x[p1_idx+1]
            dz = x[p2_idx+2] - x[p1_idx+2]
...
def Barrier_Circle_crossing():
    ...
    paths = []
    global T, dt
    T = 80 # Since the distance of the path is 12 meters we need atleast 12/0.4 = 30 timesteps to be able to find a solution that does not violate the safety distance, we add some extra timesteps in case the path gets longer. 
    dt = 1 / (T - 1)

    deadline = time.perf_counter() + TIME_LIMIT_SECONDS
    global GLOBAL_DEADLINE
    GLOBAL_DEADLINE = time.perf_counter() + TIME_LIMIT_SECONDS
    while True:
        ...
        step_constraint = NonlinearConstraint(
            lambda x: step_constraints(x, len(drones)),
            -np.inf,
            MAX_STEP**2,
            jac=lambda x: step_constraint_jac(x, len(drones))
        )
    ...
    if SAVE_FIGS:
        for i, paths in enumerate(paths):
            ...
            for j, path in enumerate(paths):
                ...
                # plot the individual points in the path as small crosses in the same colour
                ax.scatter(x, y, z, marker='x', s=10)
            ...
            if not AUTOAXIS_IN_CIRCLEPLOT:
                ...
                ax.set_zlim(0, 2)
            ax.legend()
            plt.savefig(f"SimonCircleFiguresBarrier/Circle_crossing_{results[i]['num_drones']}.png")
            if PLOT: 
                plt.show()
