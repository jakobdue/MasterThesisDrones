...
from RRT_fast import plan_multiple_paths_rrtstar, plot_paths
...
RESULTS_FILE = "RRTStarResults_test_jakob.txt"
...
def test_circle_scaling(results):
    ...
    while True:
        ...
        paths = plan_multiple_paths_rrtstar(pairs)
...
def test_fixed_missions(results):
    ...
    for name, mission in [("OG", mission_OG), ("Cross", mission_cross)]:
        ...
        start_time = time.perf_counter()
        paths = plan_multiple_paths_rrtstar(mission)
        elapsed = time.perf_counter() - start_time
